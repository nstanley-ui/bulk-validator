...
PLATFORM_SCHEMAS = {
    'Google Ads': {
        'required': ['Final URL'],  # Only URL is truly required for validation
        'recommended': ['Headline', 'Description'],
        'upload_required': ['Campaign', 'Ad Group'],  # Required for platform upload
    ...
    'Meta Ads': {
        ...
        'upload_required': [],  # Meta has different upload structure
    ...
    'LinkedIn Ads': {
        ...
        'upload_required': ['Campaign Group', 'Campaign'],  # Required for LinkedIn upload
...
def validate_schema(df: pd.DataFrame, platform: str) -> List[Dict]:
    ...
    # Check required columns (for validation)
    ...
    # Check upload-required columns (warnings only)
    for col in schema.get('upload_required', []):
        if col not in df.columns:
            issues.append({
                'type': 'upload_required',
                'severity': 'warning',
                'column': col,
                'message': f"⚠️ Upload Requirement: {col} must be filled before uploading to platform UI"
            })
        else:
            ...
            if null_count > 0:
                issues.append({
                    ...
                    'severity': 'warning',
                    'column': col,
                    'message': f"⚠️ Upload Requirement: {null_count} rows have empty {col} - fill these before platform upload"
                })
    ...
    for col in schema.get('recommended', []):
        if col not in df.columns:
            issues.append({
                ...
                'severity': 'info',
                'column': col,
                'message': f"💡 Recommended column missing: {col}"
            })
...
def analyze_row(row: pd.Series, index: int, platform: str, policy_engine: PolicyEngine, 
    ...
    # 1. SCHEMA VALIDATION - Required columns (for content validation)
    for col in schema.get('required', []):
